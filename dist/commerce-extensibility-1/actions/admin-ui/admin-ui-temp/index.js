...
function App() {
  ...
  const filtered = (0,react.useMemo)(() => articles.filter((row) => rowMatches(row, search) && (status === 'all' ? true : row.visibility === status)), [articles, search, status]);
...
(0,client.createRoot)(document.getElementById('root')).render(react.createElement(App));
